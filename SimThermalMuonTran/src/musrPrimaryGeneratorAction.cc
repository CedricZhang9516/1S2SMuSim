...
void musrPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if (takeMuonsFromTurtleFile) {
	...
	fgets(line,500,fTurtleFile);
      ...
      //changed by Cedirc for diffusion model input
	...
	exit(1);
      ...
      std::cout << z0_InitialTurtle << "\t" << xTmp << "\t" << xangle << "\t" << z0 << std::endl;
      //modified by rkita 20160531
      ...
      /*
      G4cout<<"#TEST COMMENT1#"<<G4endl;
      G4cout<<"z0 : "<<z0<<" z0_InitialTurtle : "<<z0_InitialTurtle<<G4endl;
      G4cout<<"#END1#"<<G4endl;
      */
      ...
      G4cout<<"#TEST COMMENT GENERAL#"<<G4endl;
      G4cout<< " x : "  << x  << " y : "  << y  << " xangle : "  << xangle  << " yangle : "  << yangle  <<G4endl;
      G4cout<< " x0 : " << x0 << " y0 : " << y0 << " xangle0 : " << xangle0 << " yangle0 : " << yangle0 <<G4endl;
      G4cout<< " xTmp : " << xTmp << " yTmp : " << yTmp << " xAngleTmp : " << xAngleTmp << " yAngleTmp : " << yAngleTmp <<G4endl;
      G4cout<<"#END GENERAL#"<<G4endl;
	...
	/*
      G4cout<<"#TEST COMMENT3#"<<G4endl;
      G4cout<<"p : "<<p<<" turtleMomentumP0 : "<<turtleMomentumP0<<" turtleSmearingFactor : "<<turtleSmearingFactor<<G4endl;
      G4cout<<"#END3#"<<G4endl;
	*/
    ...
    //z=z0;
    z=z0+zTmp;
          G4cout<<"x,y,z=("<<x/mm<<","<<y/mm<<","<<z/mm<<","<<zTmp/mm<<"),  angles="<<xangle/mrad<<","<<yangle/mrad<<"  p="<<p/MeV<<G4endl;
  }
...
/*
  ...
  if( (p*p -px*px -py*py)<0 ){//add 20160721 rkita
    pz = 0;
  }
  */
  //changed by Cedric based on Otani-san's toRFQstudy
  double a = 2.*3.14*G4UniformRand();
  //px = 1.0*p*sin(xangle)*sin(yangle);
  px = 1.0*p*sin(xangle)*sin(a);
  //py = 1.0*p*sin(xangle)*cos(yangle);
  py = 1.0*p*sin(xangle)*cos(a);
  pz = 1.0*p*cos(xangle);

  //px = p*sin(xangle);
  //py = p*sin(yangle);
  //if( p*p -px*px -py*py <0 )
  //p += sqrt(-1.01*(p*p -px*px -py*py) );
  pz = std::sqrt(p*p - px*px - py*py);
  std::cout << "px: " << px <<"\t"
      << "py: " << py << "\t"
      << "pz: " << pz << "\t"
...
/*
  G4cout<<"#TEST COMMENT4#"<<G4endl;
  G4cout<<" p : "<<p<<" px : "<<px<<" py : "<<py<<" pz : "<<pz<<G4endl;
  G4cout<<" xangle : "<<xangle<<" yangle : "<<yangle<<G4endl;
  G4cout<<"#END4#"<<G4endl;
  */
  // Assign spin 
  ...
  G4cout<<"particle_mass: \t"<<particle_mass << G4endl;
  //particle_mass = 1880; //for comissioning study 170318
