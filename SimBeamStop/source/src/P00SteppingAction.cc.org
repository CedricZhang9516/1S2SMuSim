...
void P00SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  if( particleName == "mu+" && aTrack->GetTrackID() == 1 ){
    //-- mu+ generated.
    if( stepNum == 1 ){
      runData->SetAllVars(step,runData->tvGenPrd,1);
    }
    //-- mu+ killed.
    if( runData->tvGenEnd.tgtLevel == 0
	&& (runData->hitTgt0 > 0 || runData->hitTgt1 > 0) ){
      runData->tvGenEnd.tgtLevel = 1;
    }
    runData->SetAllVars(step,runData->tvGenEnd,1);
    // To be updated at every step so that the last step information will be
    // held eventually without assuming what the trkStatus of the last step
    // would be.

    //-- mu+ has just hit the target at the end of this step.
    if( pstStatus == fGeomBoundary
	&& pstVol && pstVol->GetName() == "Target" ){ // Entering.
      runData->hitTgt0++;
      runData->tvTgtHit0.tgtLevel= runData->hitTgt0;
      runData->SetAllVars(step,runData->tvTgtHit0,1);
    }
    if( preStatus == fGeomBoundary
	&& preVol && preVol->GetName() == "Target" ){ // Entered.
      runData->hitTgt1++;
      runData->tvTgtHit1.tgtLevel= runData->hitTgt1;
      runData->SetAllVars(step,runData->tvTgtHit1,1);
    }
    
    //-- mu+ will exit the target in the next step.
    if( pstStatus == fGeomBoundary
	&& preVol && preVol->GetName() == "Target"
	&& pstVol && pstVol->GetName() != "Target" ){ // Exiting.
      runData->extTgt0++;
      runData->tvTgtExt0.tgtLevel= runData->extTgt0;
      runData->SetAllVars(step,runData->tvTgtExt0,1);
    }
    if( runData->extTgt0 > 0 && runData->extTgt1 == 0
	&& preStatus == fGeomBoundary
	&& preVol && preVol->GetName() != "Target" ){ // Exited.
      runData->extTgt1++;
      runData->tvTgtExt1.tgtLevel= runData->extTgt1;
      runData->SetAllVars(step,runData->tvTgtExt1,1);
    }
  }
  */
  ...
  if( particleName == "mu+" && aTrack->GetTrackID() == 1 ){
    //-- mu+ generated.
    if( stepNum == 1 ){
      runData->SetAllVars(step,runData->tvGenPrd,0,0);
    }
    //-- mu+ killed.
    if( runData->tvGenEnd.tgtLevel == 0
	&& (runData->hitTgt0 > 0 || runData->hitTgt1 > 0) ){
    ...
    runData->SetAllVars(step,runData->tvGenEnd,0,1);
    // To be updated at every step so that the last step information will be
    // held eventually without assuming what the trkStatus of the last step
    // would be.
    ...
    //-- mu+ has just exit the target at the end of this step.
    if( pstStatus == fGeomBoundary
	&& preVol && preVol->GetName() == "Target"
	&& pstVol && pstVol->GetName() != "Target" ){ // Exiting.
      runData->extTgt0++;
