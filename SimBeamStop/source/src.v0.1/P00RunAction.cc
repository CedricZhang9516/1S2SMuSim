...
P00RunAction::P00RunAction():G4UserRunAction()
{
  ...
  //-- First Ntuple, ID = 0
  mgr->CreateNtuple("P00RawData","mu+ stopped primary data");
  // Column takes values of G4double type
  mgr->CreateNtupleDColumn("XPosition"); // columnID = 0
  mgr->CreateNtupleDColumn("YPosition"); // columnID = 1
  mgr->CreateNtupleDColumn("ZPosition"); // columnID = 2
  mgr->FinishNtuple();

  //-- Second Ntuple, ID = 1
  mgr->CreateNtuple("P00BeamData", "mu+ beam information");
  mgr->CreateNtupleDColumn("KinEnergy"); // columnID = 0
  mgr->FinishNtuple();
